 ...
 *
 */
public class Recy03Adapter extends RecyclerView.Adapter<Recy03Adapter.GridViewHolder>{

    Context context;
    List<ShouyeGridBean.DataBean> listGridDa;
    public Recy03Adapter(Context context, List<ShouyeGridBean.DataBean> listGridDa) {
    ...
    public void onBindViewHolder(GridViewHolder holder, int position) {
        if(listGridDa.size()>0) {
            ImageLoader.getInstance().displayImage(listGridDa.get(position+4).getIcon(), holder.recy02_image);
            holder.recy02_text.setText(listGridDa.get(position+4).getName());
        }
    }
